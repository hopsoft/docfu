import {basename, dirname, join, realpath} from './file-system.js'
...
const env = {
  ...
  set source(path) {
    ...
    if (!source) {
      bus.pub('missing', {file: __file, source: path})
      process.exit(1)
    }
    process.env.DOCFU_SOURCE = source
    bus.pub('info', {file: __file, source})
    return source
  },
   ...
   */
  set sandbox(path = '.docfu') {
    process.env.DOCFU_SANDBOX = path
    bus.pub('info', {file: __file, sandbox: path})
    return path
  },
