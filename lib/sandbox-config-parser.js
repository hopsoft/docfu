 ...
 * Uses global docfu.source.directory
 * @returns {{master: Object, hierarchical: Array}} Merged master config and hierarchical configs
 */
function collectConfigs() {
  const source = docfu.source.directory
  const paths = source.glob('**/docfu.yml', {
    dot: false,
  ...
  for (const path of paths) {
    const file = source.join(path)
    const content = file.read()
    if (!content) continue

    const config = yaml.load(content) || {}
    const dir = file.directory.relative(source) || '.'
    configs.push({dir, config, file: String(file)})
  }
  ...
  for (const {dir, config} of configs) {
    if (config.exclude?.length) {
      const prefixed = config.exclude.map(pattern => {
        if (dir === '.') return pattern
        const joined = new Pathname(dir).join(pattern)
    ...
    if (config.unlisted?.length) {
      const prefixed = config.unlisted.map(pattern => {
        if (dir === '.') return pattern
        const joined = new Pathname(dir).join(pattern)
        return String(joined).replace(/\\/g, '/')
      })
...
function getFrontmatter(configs, file) {
  const filePath = new Pathname(file)
  const source = docfu.source.directory

  if (!filePath.exists || !source.exists) return {defaults: {}, specific: null}

  const rel = filePath.relative(source)
  const fileDir = filePath.directory.relative(source) || '.'
...
function isExcluded(file, patterns) {
  ...
  const relPath = new Pathname(file).relative(docfu.source.directory)
  return new Pathname(relPath).matches(...patterns)
}
