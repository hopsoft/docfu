...
function updatePartials(dir, conversions) {
  ...
  for (const file of files) {
    ...
    const updated = content.replace(pattern, (match, partialPath) => {
      ...
      if (conversions.has(normalized)) {
        ...
        hasChanges = true
        bus.pub('info', {FILE, partial: {from: partialPath, to: newPath}})
  ...
  if (count > 0) bus.pub('success', {FILE, updated: `partials in ${count} file(s)`})
}
  ...
  for (const src of files) {
    ...
    if (basename?.match(/^readme$/i)) {
      ...
      if (!hasIndex) {
        ...
        bus.pub('info', {FILE, rename: {from: `${basename}${ext}`, to: `index${ext}`}})
      }
    }
