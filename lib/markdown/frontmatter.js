...
function mergeFrontmatter(path, content, opts = {}) {
  bus.pub('call', {FILE, mergeFrontmatter: {path, content, opts}})
  ...
  if (opts.isUnlisted) {
    ...
    bus.pub('omit', {FILE, mergeFrontmatter: {path}})
  }

  // Ensure title exists
  if (!frontmatter.title) {
    frontmatter.title = getTitle(path, body)
    bus.pub('memo', {FILE, mergeFrontmatter: {title: frontmatter.title}})
  }

  const serialized = serializeFrontmatter(frontmatter, body, {removeH1: true})
  bus.pub('done', {FILE, mergeFrontmatter: {serialized}})
  return serialized
}
