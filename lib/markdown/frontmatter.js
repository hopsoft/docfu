...
import {getApplicable} from '../config.js'
 ...
 * @param {Array} [options.configs] - Hierarchical configs from discover()
...
function mergeFrontmatter(path, content, options = {}) {
  ...
  if (options.configs) {
    const {defaults, specific} = getApplicable(options.configs, path)
  ...
  if (options.isUnlisted) {
    ...
    console.info(theme.lead('→ unlisted'), theme.muted(path))
  }

  // Ensure title exists
  if (!frontmatter.title) {
    frontmatter.title = getTitle(path, body)
    console.info(theme.lead('→ title'), frontmatter.title, theme.muted(path))
  }
