...
import {dirname, join, read, realpath} from './file-system.js'
import env from './env.js'
import theme from './theme.js'

/**
 * Discover and merge all docfu.yml configuration files
 * @returns {{master: Object, hierarchical: Array}} Merged master config and hierarchical configs
 */
function discover() {
  console.info(theme.lead('→ discover'), 'config files')
  ...
  for (const path of paths) {
    const file = join(env.source, path)
    const content = read(file)
    if (!content) continue

    const config = yaml.load(content) || {}
    const dir = dirname(file).replace(env.source, '').replace(/^\//, '') || '.'
  ...
  if (configs.length) console.info(theme.success(`✓ ${configs.length} config file(s)`))
  return {master, hierarchical: configs}
}

/**
 * Get applicable frontmatter configuration for a specific file
 * @param {Array} configs - Hierarchical configs from discover()
 * @param {string} file - Absolute file path
 * @returns {{defaults: Object, specific: Object|null}} Merged config
 */
function getApplicable(configs, file) {
  const realFile = realpath(file)
  const realSource = realpath(env.source)
  if (!realFile || !realSource) return {defaults: {}, specific: null}

  const rel = realFile.replace(realSource, '').replace(/^\//, '')
  const fileDir = dirname(join(env.source, rel)).replace(env.source, '').replace(/^\//, '') || '.'
...
export {discover, getApplicable, isExcluded, isUnlisted}
